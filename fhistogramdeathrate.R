...
# Create a histogram for Death Rates with counts on Y-axis and overlay a normal curve
ggplot(data, aes(x = Death_Rate)) +
  geom_histogram(binwidth = 20, fill = "skyblue", color = "black", alpha = 0.7) +
  stat_function(fun = function(x) {
    # Scale normal curve to match the histogram counts
    dnorm(x, mean = mean(data$Death_Rate, na.rm = TRUE),
          sd = sd(data$Death_Rate, na.rm = TRUE)) *
      nrow(data) * 10  # Scale by total data points and binwidth
  }, color = "red", size = 1) +
