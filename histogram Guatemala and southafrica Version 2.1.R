...
x <- seq(min(filtered_data$Death_Rate, na.rm = TRUE), max(filtered_data$Death_Rate, na.rm = TRUE), length = 100)
y <- dnorm(x, mean = mean_value, sd = sd_value) * nrow(filtered_data) * bin_width
bell_curve_data <- data.frame(x = x, y = y)
...
ggplot(filtered_data, aes(x = Death_Rate)) +
  ...
  geom_line(data = bell_curve_data, aes(x = x, y = y), color = "red", size = 1) +
  labs(
